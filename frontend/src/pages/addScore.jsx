...
const BulkUpload = () => {
            ...
            if (updatedData[i].player_name) {
                try {
                    const response = await axios.get(
                        `https://scorepoint.onrender.com/api/teams/player/search?name=${updatedData[i].player_name}`
                    );
    ...
    const handleSubmit = async () => {
        ...
        try {
            await axios.post(
                "https://scorepoint.onrender.com/api/points/player/submit_batch",
                payload,
        ...
        <div className="max-w-4xl mx-auto p-20 bg-white rounded-lg shadow-lg">
            ...
            <div className="border-dashed border-2 border-gray-300 p-4 rounded-lg text-center cursor-pointer">
                ...
                <label htmlFor="fileUpload" className="flex flex-col items-center cursor-pointer">
                    ...
                    <span className="text-gray-600">Click or Drag & Drop to Upload Excel File</span>
                </label>
            </div>
            {fileName && <p className="mt-2 text-sm text-gray-700">Uploaded File: {fileName}</p>}
                                ...
                                <tr key={rowIndex} className="border border-gray-300">
                                    <td className="p-2">
                                        <input
                                            ...
                                            onChange={(e) => handleInputChange(rowIndex, "player_name", e.target.value)}
                                            ...
                                            <select onChange={(e) => handleAssignPlayerId(rowIndex, e.target.value)}>
                                                ...
                                                {row.possible_players.map((player) => (
                                                    <option key={player.player_id} value={player.player_id}>
                                                        {player.player_name} - {player.team_name} (ID: {player.player_id})
                                                    </option>
                                                ))}
                                            </select>
                                        ) : (
                                    ...
                                    <td className="p-2">
                                        <input
                                            type="number"
                                            value={row.sport_id}
                                            onChange={(e) => handleInputChange(rowIndex, "sport_id", e.target.value)}
                                            className="w-full p-2 border rounded-md"
                                        />
                                    </td>
                                    <td className="p-2">
                                    ...
                                    <td className="p-2">
                                        <input
                                            ...
                                            onChange={(e) => handleInputChange(rowIndex, "competition_level", e.target.value)}
                                            className="w-full p-2 border rounded-md"
                                        />
                                    </td>
                                    <td className="p-2">
                                        <input
                                            type="number"
                                            value={row.points}
                                            onChange={(e) => handleInputChange(rowIndex, "points", e.target.value)}
                                            className="w-full p-2 border rounded-md"
                                        />
                                    </td>
                                </tr>
            ...
            <div className="flex justify-between mt-4">
                <button className="bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700" onClick={fetchPlayerIds}>
                    Fetch IDs
                </button>
                <button className="bg-green-600 text-white py-2 px-4 rounded-lg hover:bg-green-700" onClick={handleSubmit}>
                    Submit Data
                </button>
            </div>
        </div>
