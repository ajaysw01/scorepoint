...
const sportsCategories = {
  cricket:["none"],
  Badminton: ["men_singles","men_doubles","women_singles","women_doubles","mixed_doubles"],
  TableTennis: ["men_singles","men_doubles","women_singles","women_doubles","mixed_doubles"],
  Carrom: ["men_singles","men_doubles","women_singles","women_doubles","mixed_doubles"],
  Darts: ["none"],
};

const Updates = () => {
  const [currentSport, setCurrentSport] = useState("cricket");
  const [currentCategory, setCurrentCategory] = useState(sportsCategories["cricket"][0]);
        ...
        <div className="flex overflow-x-auto space-x-2 px-2">
          {sportsCategories[currentSport].map((category) => (
            <button
              ...
              className={`whitespace-nowrap px-4 py-2 text-sm font-medium rounded-lg transition-all ${
                currentCategory === category ? "bg-red-500 text-white" : "bg-gray-200 text-gray-700"
              }`}
            >
              {category}
            </button>
