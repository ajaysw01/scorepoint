...
// Import Animations
import cricketAnimation from '/static/animations/cricket.json';
import badmintonAnimation from '/static/animations/badminton.json';
import tableTennisAnimation from '/static/animations/tabletennis.json';
import carromAnimation from '/static/animations/carrom.json';
import dartsAnimation from '/static/animations/darts.json';

// Sports Config with Default Frames
const sports = [
  { name: 'Cricket', animation: cricketAnimation, path: '/cricket', defaultFrame: 50 },
  { name: 'Badminton', animation: badmintonAnimation, path: '/badminton', defaultFrame: 15 },
  { name: 'Table Tennis', animation: tableTennisAnimation, path: '/tabletennis', defaultFrame: 50 },
  { name: 'Carrom', animation: carromAnimation, path: '/carrom', defaultFrame: 50 },
  { name: 'Darts', animation: dartsAnimation, path: '/Darts/6/none', defaultFrame: 50 },
  { name: 'Fun Friday', image: 'static/images/funfriday.png', path: '/Fun Friday/5/none' }, // Static Image
];

const Scores = () => {
  return (
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6">
      <h1 className="text-3xl sm:text-4xl font-bold text-center text-black mb-6">Choose Your Sport</h1>
    ...
    <Link to={sport.path} key={sport.name} className="group" aria-label={`Go to ${sport.name}`}>
      <div className="relative overflow-hidden rounded-2xl border border-gray-300 hover:border-gray-500 shadow-lg hover:shadow-xl hover:scale-105 transition-transform duration-300 bg-white">
        ...
        <div className="w-full h-60 sm:h-72 flex items-center justify-center p-4 sm:p-2">
          {sport.animation ? (
            <HoverLottie animationData={sport.animation} speed={1} defaultFrame={sport.defaultFrame} />
          ) : (
            <img
              src={sport.image}
              alt={sport.name}
